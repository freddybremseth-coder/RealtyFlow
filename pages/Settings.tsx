...
const Settings: React.FC = () => {
  ...
  const handleAdvisorImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    const b64 = await toBase64(file);
    setProfile(p => p ? ({ ...p, imageUrl: b64 }) : null);
  };
  ...
  const handleBrandSave = async (updated: Partial<Brand>) => {
    ...
    if (!user) { console.error('User not auth'); return; }
    const dataToSave = id ? rest : { ...rest, user_id: user.id };
  ...
  const addExpertise = () => {
    ...
    if (trimmed && !profile.expertise.includes(trimmed)) {
      setProfile(p => p ? ({ ...p, expertise: [...p.expertise, trimmed] }) : null);
      setExpertiseInput('');
    }
  };
  ...
  useEffect(() => {
      if (automation?.emailFromEmail) {
          setTestAddress(automation.emailFromEmail);
      }
  }, [automation]);
  ...
  const handleEmailTest = async () => {
    ...
    const result = await emailService.send({
      ...
      text: `Test fra ${automation.emailFromName} <${automation.emailFromEmail}>`,
    });
    setEmailTesting(false);
    setEmailTestResult(result.success ? { ok: true, msg: `Sendt! ID: ${result.id}` } : { ok: false, msg: result.error ?? 'Feil' });
  };
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-4 py-6 lg:py-10 space-y-8 pb-24 animate-in fade-in duration-500">
      {editingBrand && (
        <BrandEditModal brand={editingBrand} onSave={handleBrandSave} onClose={() => setEditingBrand(null)} onDelete={handleBrandDelete} />
      )}
        ...
        {TABS.map(tab => (
          <button
            ...
            className={`flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all whitespace-nowrap flex-1 ${
              activeTab === tab.id ? 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/30' : 'text-slate-500 hover:text-slate-300'
            }`}>
            {tab.icon} {tab.label}
          </button>
        ))}
      ...
      {activeTab === 'brands' && (
        <div className="space-y-4 animate-in fade-in">
          <div className="flex justify-end">
            <button onClick={() => setEditingBrand({})} className="flex items-center gap-2 px-4 py-2 bg-cyan-500 hover:bg-cyan-400 text-slate-950 rounded-xl text-sm font-bold transition-colors">
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-4">
                  <div className="w-16 h-16 rounded-xl bg-slate-800 border border-slate-700 flex items-center justify-center overflow-hidden shrink-0">
                    {brand.logo_url ? <img src={brand.logo_url} className="w-full h-full object-contain p-1" alt={brand.name} /> : <Building2 size={24} className="text-slate-500" />}
                  </div>
                  <div>
                ...
                <button onClick={() => setEditingBrand(brand)} className="flex items-center gap-2 px-4 py-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 hover:border-cyan-500/40 text-slate-300 hover:text-cyan-400 rounded-xl text-sm transition-all shrink-0">
      ...
      {activeTab === 'profile' && (
        <form onSubmit={handleProfileSubmit} className="space-y-6 animate-in fade-in">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
            <div className="flex items-center gap-5">
              <div className="relative group cursor-pointer shrink-0" onClick={() => advisorFileRef.current?.click()}>
                <div className="w-24 h-24 rounded-full bg-slate-800 border-2 border-slate-700 flex items-center justify-center overflow-hidden">
                  {profile.imageUrl ? <img src={profile.imageUrl} className="w-full h-full object-cover" alt="profile" /> : <User size={32} className="text-slate-500" />}
                </div>
              ...
              <div className="flex-1 space-y-3">
                <div>
                  ...
                  <input className={iCls} value={profile.name} onChange={e => setProfile(p => p ? { ...p, name: e.target.value } : null)} placeholder="Ola Nordmann" />
                </div>
                <div>
                  <label className={lCls}>Tittel</label>
                  <input className={iCls} value={profile.title} onChange={e => setProfile(p => p ? { ...p, title: e.target.value } : null)} placeholder="CEO & Founder" />
                </div>
              </div>
            </div>
          </div>
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className={lCls}>E-post</label>
                <input type="email" className={iCls} value={profile.email} onChange={e => setProfile(p => p ? { ...p, email: e.target.value } : null)} placeholder="din@epost.no" />
              </div>
              <div>
                <label className={lCls}>Telefon</label>
                <input type="tel" className={iCls} value={profile.phone} onChange={e => setProfile(p => p ? { ...p, phone: e.target.value } : null)} placeholder="+47 123 45 678" />
              </div>
            </div>
          </div>
      ...
      {activeTab === 'email' && (
        <div className="space-y-6 animate-in fade-in">
          ...
          <form onSubmit={handleEmailSave} className="space-y-5">
             <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
              <h3 className="text-sm font-bold text-slate-200 flex items-center gap-2"><Key size={14} className="text-cyan-400" /> Resend API-nøkkel</h3>
              <div>
                <label className={lCls}>API-nøkkel</label>
                <div className="relative">
                  <input type={showResendKey ? 'text' : 'password'} className={iCls + ' pr-10 font-mono'} placeholder="re_xxxxxxxxxxxxxxxxxxxx" value={automation.resendApiKey || ''} onChange={e => setAutomation(a => a ? { ...a, resendApiKey: e.target.value } : null)} />
                  <button type="button" onClick={() => setShowResendKey(s => !s)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-300 transition-all">
             ...
             <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
              <h3 className="text-sm font-bold text-slate-200 flex items-center gap-2"><Mail size={14} className="text-cyan-400" /> Avsenderinfo</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className={lCls}>Avsendernavn</label>
                  <input type="text" className={iCls} placeholder="Freddy – Soleada.no" value={automation.emailFromName || ''} onChange={e => setAutomation(a => a ? { ...a, emailFromName: e.target.value } : null)} />
                </div>
                <div>
                  <label className={lCls}>Avsenderadresse</label>
                  <input type="email" className={iCls} placeholder="freddy@soleada.no" value={automation.emailFromEmail || ''} onChange={e => setAutomation(a => a ? { ...a, emailFromEmail: e.target.value } : null)} />
                </div>
              </div>
            </div>
            <button type="submit" className="flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold bg-cyan-500 text-slate-950 hover:bg-cyan-400 transition-all">
              {emailSaved ? <><CheckCircle2 size={14} /> Lagret!</> : <><Save size={14} /> Lagre e-postinnstillinger</>}
            </button>
          </form>
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
            <h3 className="text-sm font-bold text-slate-200 flex items-center gap-2"><Send size={14} className="text-cyan-400" /> Send test-e-post</h3>
            <div className="flex gap-3">
              <input type="email" className={iCls + ' flex-1'} placeholder="din@epost.no" value={testAddress} onChange={e => setTestAddress(e.target.value)} />
              <button type="button" onClick={handleEmailTest} disabled={emailTesting || !testAddress || !automation.resendApiKey} className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold bg-indigo-500/10 border border-indigo-500/25 text-indigo-400 hover:bg-indigo-500/20 transition-all disabled:opacity-40 whitespace-nowrap">
                {emailTesting ? <><Loader2 size={12} className="animate-spin" /> Sender…</> : <><Send size={12} /> Send test</>}
              </button>
            </div>
            {emailTestResult && (
              <div className={`flex items-start gap-2 px-4 py-3 rounded-xl text-xs border ${emailTestResult.ok ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}>
                {emailTestResult.ok ? <CheckCircle2 size={13} /> : <AlertTriangle size={13} />}
                {emailTestResult.msg}
              </div>
            )}
          </div>
      ...
      {activeTab === 'system' && (
        <div className="space-y-6 animate-in fade-in">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
            <h3 className="text-sm font-bold text-slate-200 flex items-center gap-2"><Languages size={14} className="text-cyan-400" /> Språk i appen</h3>
            <p className="text-xs text-slate-500">Velg språk for brukergrensesnittet.</p>
